...
"""
@author:Dr.Bai Yunpeng(baiyunpeng@snbigdata.cn)
@description: 多模型混合图像相似度检测器
"""

"""
多模型图像相似度检测器
解决单一CLIP模型相似度过高的问题
python weighted_models_demo.py --target 
 --directory /Users/cloudbai/PycharmProjects/imagesim/similarities/examples/data/shanqi4000_test_1
python weighted_models_demo.py --target /Users/cloudbai/PycharmProjects/imagesim/similarities/examples/data/image8-like-image1.png --directory /Users/cloudbai/PycharmProjects/imagesim/similarities/examples/data/shanqiimage


python weighted_models_demo.py --target 【目标单个图像路径】 --directory 【待检测图像数据集路径】
...
class MultiModelSimilarityDetector:
    ...
    def _calculate_adaptive_weights(self, similarities: Dict[str, float]) -> Dict[str, float]:
        """根据特征质量自适应计算权重"""
        base_methods = [ 'resnet', 'vit', 'traditional', 'sift']
...
def main():
    ...
    parser.add_argument("--target", "-t", type=str, required=True,default='./examples/data/shanqi4000_test_1/new17503489693520.jpg',
                        help="目标图片路径")
    parser.add_argument("--directory", "-d", type=str, required=True,default='./examples/data/shanqi4000_test_1',
                        help="图片目录路径")
    parser.add_argument("--threshold", "-th", type=float, default=0.5,
                        help="相似度阈值")
    parser.add_argument("--method", "-m", type=str, default='weighted_fusion',
                        choices=['resnet', 'vit', 'traditional', 'sift',
    ...
    for i, (image_path, score, details) in enumerate(similar_images, 1):
        ...
        print(f"   相似度 ({args.method}): {score:.4f}")
